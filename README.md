...
This repo originally shipped as a zero-dependency HTML/CSS/JS catalog and was
later migrated to the Next.js stack above. The pre-migration baseline is
preserved as the **`v0-vanilla`** tag — a single, much smaller, framework-free
build that opens directly via `file://` with no `npm install` required.

To browse or check out the vanilla baseline:

```bash
git checkout v0-vanilla        # detached HEAD at the vanilla tree
# (then double-click index.html in your file manager)

git checkout main              # back to the current Next.js build
```
